...
function cors(origin: string) { return { "Access-Control-Allow-Origin": origin, "Access-Control-Allow-Headers": "content-type,x-eundong-sync-token", "Access-Control-Allow-Methods": "POST,OPTIONS", "Vary": "Origin" }; }
...
function numeric(value: unknown, min: number, max: number, nullable = false): number | null { if ((value === null || value === "" || value === undefined) && nullable) return null; const n = typeof value === "number" ? value : Number(value); if (!Number.isFinite(n) || n < min || n > max) throw new Error("INVALID_NUMBER"); return n; }
function integer(value: unknown, min: number, max: number) { const n = numeric(value, min, max); if (!Number.isInteger(n)) throw new Error("INVALID_INTEGER"); return n as number; }
async function tokenHash(token: string) { const digest = await crypto.subtle.digest("SHA-256", new TextEncoder().encode(token)); return Array.from(new Uint8Array(digest)).map(v => v.toString(16).padStart(2, "0")).join(""); }
...
export default {
  fetch: withSupabase({ auth: "none" }, async (req, ctx) => {
    ...
    if (token.length < 40 || token.length > 256) return response(origin, { error: "INVALID_SYNC_TOKEN" }, 401);
    const digest = await tokenHash(token);
    const { data: access, error: accessError } = await ctx.supabaseAdmin.from("eundong_access").select("id").eq("id", 1).eq("token_hash", digest).maybeSingle();
    if (accessError || !access) return response(origin, { error: "INVALID_SYNC_TOKEN" }, 401);
    ...
    try {
      ...
      if (action === "get_day") {
        ...
        const [settings, record, feeds, meals, previous] = await Promise.all([
          ...
          db.from("eundong_daily_feeds").select("recorded_date,feed_slot,feed_id,feed_name_snapshot,moisture_snapshot,kcal_per_kg_snapshot").lt("recorded_date", input.date).order("recorded_date", { ascending: false }).order("feed_slot", { ascending: true }).limit(3),
        ]);
      ...
      if (action === "update_settings") {
        ...
        query = db.from("eundong_settings").upsert({ id: 1, pet_name: "은동", goal_weight_kg: numeric(s.goal_weight_kg, .1, 20, true), goal_start_weight_kg: numeric(s.goal_start_weight_kg, .1, 20, true), goal_start_date: s.goal_start_date, goal_end_date: s.goal_end_date }, { onConflict: "id" }).select().single();
        ...
        if (term.length < 2 || term.length > 80) return response(origin, { data: [] });
        ...
        const { data: feed, error } = await db.from("feeds").select("id,제품명,수분,final_me,official_me,corrected_me,verified,searchable_before_review").eq("id", input.feed_id).eq("type", "wet").or("verified.eq.true,searchable_before_review.eq.true").single();
        if (error || !feed) throw new Error("FEED_NOT_FOUND");
        const kcal = feed.final_me ?? feed.official_me ?? feed.corrected_me ?? null;
        query = db.from("eundong_daily_feeds").upsert({ recorded_date: input.date, feed_slot: slot, feed_id: feed.id, feed_name_snapshot: feed["제품명"], moisture_snapshot: feed["수분"], kcal_per_kg_snapshot: kcal }, { onConflict: "recorded_date,feed_slot" }).select().single();
        ...
        const rows = feeds.map((f: any) => {
          ...
          return { recorded_date: input.date, feed_slot: integer(f.feed_slot, 1, 3), feed_id: f.feed_id, feed_name_snapshot: String(f.feed_name_snapshot).slice(0, 500), moisture_snapshot: numeric(f.moisture_snapshot, 0, 100, true), kcal_per_kg_snapshot: numeric(f.kcal_per_kg_snapshot, 0, 100000, true) };
        });
        ...
        if (feedSlot) {
          ...
          snapshot = { feed_slot: feedSlot, feed_id: f.feed_id, feed_name_snapshot: f.feed_name_snapshot, moisture_snapshot: f.moisture_snapshot, kcal_per_kg_snapshot: f.kcal_per_kg_snapshot };
        } else {
          snapshot = { feed_slot: null, feed_id: null, feed_name_snapshot: null, moisture_snapshot: null, kcal_per_kg_snapshot: null };
        }
        query = db.from("eundong_meals").upsert({ recorded_date: input.date, meal_slot: slot, meal_time: TIMES[slot - 1], ...snapshot, amount_g: numeric(input.amount_g, 0, 5000), added_water_ml: numeric(input.added_water_ml, 0, 5000) }, { onConflict: "recorded_date,meal_slot" }).select().single();
      ...
      return response(origin, { data: result.data });
    } catch (error) {
      const message = error instanceof Error ? error.message : "UNKNOWN";
      const bad = message.startsWith("INVALID") || message === "FEED_NOT_FOUND";
      console.error(message);
      return response(origin, { error: bad ? message : "DATABASE_ERROR" }, bad ? 400 : 500);
    }
  }),
};
